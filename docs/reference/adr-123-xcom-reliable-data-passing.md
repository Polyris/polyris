...
**Cross-record failure semantics.** The two records are written independently by two SFN states (`Init_Output_Row`, then `Save_Input_Record`), each with a `States.ALL` Catch that logs and continues rather than failing the task. This is deliberate: `input#` is Console-preview-only (never read at runtime by downstream tasks or by the SDK), so a lost `input#` write only degrades a UI snapshot — the task still runs, produces its `result`, and downstream `xcom.get()` reads succeed. Conversely, a lost `Init_Output_Row` write leaves the wrapper without a canonical row until `Save_Canonical_Output` `putItem`-creates it at task end (or `Save_Canonical_Output_Preserve` `updateItem`s it if the task called `xcom.push()`); concurrent same-date reads during that window get `XComMissingError`, which is the correct semantics ("not yet run").

**Marker refresh contract (CLAUDE.md #31).** `_write_synthetic_output_marker` in `console_api` is a second writer of the same canonical row alongside the wrapper. Its guard is split into two concerns: (1) protect real user output from clobber — GetItem the row, if `result` is opaque user data, skip and emit a `_notify_warn_*` so the operator sees the mismatch; (2) refresh stale marker — if `result` is absent OR is another marker this writer produced, unconditional Update. Pre-split, `attribute_not_exists(#r)` blocked both concerns silently at INFO log level, and a second same-date manual action left the first action's operator/reason/resolution on the row forever. `_pipeline_execution` is now stamped on every marker so the SDK's `XComManuallyResolvedError` message names which run created it — makes cross-run bleed on shared canonical rows self-diagnosing.

**UI cross-run staleness gate.** Because the canonical row is date-scoped, a settled task that never ran the wrapper (resolved via UI before `Save_Canonical_Output` fires) leaves the row stamped with a prior same-date run's `run_id`. `OutputCard` and `InputSection` gate on both (a) `task.status ∈ TASK_SETTLED_STATUSES` and (b) `row.run_id === task.run_task_helper_arn`; a mismatch on either renders a muted empty state ("pending" or "from prior run") instead of attributing another run's content to this one. Backend `get_task_output` returns the three run_ids needed for the comparison. No cross-record atomic guarantee is attempted because none is needed — the reader contracts (SDK + Console) tolerate either record being absent.
