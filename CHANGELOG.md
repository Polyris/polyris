...
- **Canonical-row read gated on settled state.** The `output#{pipeline}#{task}#{date}` row is shared across every same-date run (backfill semantics), so a task in a non-settled status (`waiting_decision`, `running`, etc.) can inherit a prior run's marker/output visually. OutputCard now checks `task.status ∈ TASK_SETTLED_STATUSES` — non-settled tasks render a muted "pending" state with the current status inline, instead of showing another run's content as if it were this run's. Copy button still available for advanced multi-run diagnosis. See CLAUDE.md rule #30.
